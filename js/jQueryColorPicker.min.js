...
function(a, b) {
    ...
    var I, J, K, L, M, N, O = a.ColorPicker,
        ...
        V = 1,
        W = {}, X = {}, Y = !0,
        Z = {}, $ = {}, _ = {}, ab = {}, bb = {}, cb = {}, db = "AnimationFrame",
        eb = "request" + db,
        fb = "cancel" + db,
        gb = ["ms", "moz", "webkit", "o"],
        hb = function(a) {
            ...
            var d = function(b, c) {
              ...
              // window.o = this;
                var d = this,
                    ...
                    g = b.RND.rgb,
                    h = b.RND.hsl,
                    i = d.isIE8 ? (b.alpha < .16 ? "0" : "") + Math.round(100 * b.alpha).toString(16).toUpperCase() + b.HEX : "",
                    j = g.r + ", " + g.g + ", " + g.b,
                    k = "rgba(" + j + ", " + b.alpha + ")",
                    l = 1 !== b.alpha && !d.isIE8,
                    m = e.data("colorMode");
                f.css({
                    color: b.rgbaMixCustom.luminance > .22 ? "#222" : "#ddd",
                    "background-color": k,
                    filter: d.isIE8 ? "progid:DXImageTransform.Microsoft.gradient(startColorstr=#" + i + ",endColorstr=#" + i + ")" : ""
                }), e.val("HEX" !== m || l ? "rgb" === m || "HEX" === m && l ? l ? k : "rgb(" + j + ")" : "hsl" + (l ? "a(" : "(") + h.h + ", " + h.s + "%, " + h.l + "%" + (l ? ", " + b.alpha : "") + ")" : "#" + (d.isIE8 ? i : b.HEX)), d.displayCallback && d.displayCallback(b, c, d)
            }, e = function(b, c) {
                    var d = this,
                        e = i.current;
                    if ("toMemery" === c) {
                        for (var f, g = e.nodes.memos, h = "", j = 0, k = [], l = 0, m = g.length; m > l; l++) f = a(g[l]), h = f.css("background-color"), j = Math.round(100 * f.css("opacity")) / 100, k.push(h.replace(/, /g, ",").replace("rgb(", "rgba(").replace(")", "," + j + ")"));
                        k = "'" + k.join("','") + "'", a.docCookies("colorPickerMemos" + (d.noAlpha ? "NoAlpha" : ""), k)
                    } else if ("resizeApp" === c) a.docCookies("colorPickerSize", e.color.options.currentSize);
                    else if ("modeChange" === c) {
                        var n = e.color.options.mode;
                        a.docCookies("colorPickerMode", n.type + "-" + n.z)
                    }
                }, f = function(c, f) {
                    var g = {
                        klass: b.ColorPicker,
                        ...
                        mode: a.docCookies("colorPickerMode") || "hsv-h",
                        memoryColors: a.docCookies("colorPickerMemos" + ((f || {}).noAlpha ? "NoAlpha" : "")),
                        size: a.docCookies("colorPickerSize") || 1,
                        renderCallback: d,
                        actionCallback: e
                    };
                    for (var h in f) g[h] = f[h];
                    return new g.klass(g)
                }, g = function(d, e, g) {
                    var j = g ? "off" : "on";
                    a(d)[j]("focus.colorPicker", function() {
                        var b = a(this),
                            g = b.offset(),
                            j = e ? a(h).index(this) : 0,
                            k = i[j] || (i[j] = f(this, c)),
                            l = k.color.options,
                            m = a.ui && l.draggable ? a(k.nodes.colorPicker).draggable({
                                cancel: "." + l.CSSPrefix + "app div"
                            }) : a(k.nodes.colorPicker);
                        l.color = d.value, m.css({
                            position: "absolute",
                            left: g.left + l.margin.left,
                            top: g.top + +b.outerHeight(!0) + l.margin.top
                        }), e || (l.input = d, l.patch = d, k.setColor(d.value, void 0, void 0, !0), k.saveAsBackground()), i.current = i[j], a(l.appendTo || document.body).append(m), setTimeout(function() {
                            m.show(k.color.options.animationSpeed)
                        }, 0)
                    }), a(b)[j]("mousedown.colorPicker", function(b) {
                        var c = i.current,
                            d = a(c ? c.nodes.colorPicker : void 0),
                            e = c ? c.color.options.animationSpeed : 0,
                            f = a(b.target).closest(".cp-app")[0],
                            g = a(h).index(b.target);
                        f && c && a(i).index(f) ? b.target === c.nodes.exit && (d.hide(e), a(":focus").trigger("blur")) : -1 !== g || d.hide(e)
                    })
                }, h = this,
                i = a.fn.colorPicker.colorPickers || [],
                j = new b.Colors({
                    customBG: c && c.customBG || "#FFFFFF",
                    allMixDetails: !0
                });
            return a.fn.colorPicker.colorPickers = i, a(this).each(function(d, e) {
                if (a(e).off(".colorPicker"), a(b).off(".colorPicker"), "destroy" !== c) {
                    var f = e.value.split("(");
                    a(e).data("colorMode", f[1] ? f[0].substr(0, 3) : "HEX"), g(e, c && c.multipleInstances, !1), c && c.readOnly && (e.readOnly = !0), j.setColor(e.value), c && c.init && c.init(e, j.colors)
                }
            }), this
        }
    }), a.docCookies = function(a, b, c) {
        var d, e, f, g, h = encodeURIComponent,
            i = decodeURIComponent,
            j = {};
        if (void 0 === b) {
            for (d = document.cookie.split("; ") || [], e = d.length; e--;) f = d[e].split("="), f[0] && (j[i(f.shift())] = i(f.join("=")));
            return a ? j[a] : j
        }
        c = c || {}, ("" === b || c.expires < 0) && (c.expires = -1), void 0 !== c.expires && (g = new Date, g.setDate(g.getDate() + c.expires)), document.cookie = h(a) + "=" + h(b) + (g ? "; expires=" + g.toUTCString() : "") + (c.path ? "; path=" + c.path : "") + (c.domain ? "; domain=" + c.domain : "") + (c.secure ? "; secure" : "")
    }
}(jQuery, this);
